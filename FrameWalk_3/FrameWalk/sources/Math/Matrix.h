...
struct Matrix {
	...
	// ワールド行列を作成します。
	static Matrix CreateWorld(
		const Vector3& scale, const Matrix& rotation, const Vector3& translation
		);
	// ワールド行列を作成します。
	static Matrix CreateWorld(
		const Vector3& position, const Vector3& forward, const Vector3& up
		);
	...
	// 3Dスケール / 回転 / 変換(SRT) Matrix から
	// スカラー、変換、回転のコンポーネントを取得します
