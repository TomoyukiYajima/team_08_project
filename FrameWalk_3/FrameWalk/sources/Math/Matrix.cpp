...
Matrix Matrix::CreateLookAt(const Vector3 & position, const Vector3 & target, const Vector3 & up)
{
	...
	return Matrix(
		...
		x.y, y.y, z.y, 0.0f,
		x.z, y.z, z.z, 0.0f,
		-Vector3::Dot(position, x), -Vector3::Dot(position, y), 
		-Vector3::Dot(position, z), 1.0f
		);
}

Matrix Matrix::CreateLookAt(const Matrix & pose) { return Invert(pose); }
...
Matrix Matrix::Invert(const Matrix & m)
{
	...
	if (det == 0.0f) return m;	//ãtçsóÒÇ™ë∂ç›ÇµÇ»Ç¢èÍçáÇÕï‘Ç∑
...
Matrix Matrix::Lerp(const Matrix & value1, const Matrix & value2, float amount) 
{
	...
	Quaternion rotate = 
		Quaternion::Slerp(value1.Rotation(), value2.Rotation(), amount);
	Vector3 translation = 
		Vector3::Lerp(value1.Translation(), value2.Translation(), amount);
...
Vector3 Matrix::Forward() const { return Vector3(m[2][0], m[2][1], m[2][2]); }
...
Vector3 Matrix::Backward() const { return -Forward(); }

Matrix & Matrix::Backward(const Vector3 & backward) { return Forward(-backward); }
...
Vector3 Matrix::Down() const { return -Up(); }

Matrix & Matrix::Down(const Vector3 & down) { return Up(-down); }

Vector3 Matrix::Left() const { return Vector3(m[0][0], m[0][1], m[0][2]); }
...
Matrix & Matrix::Right(const Vector3 & rignt) { return Left(-rignt); }

Vector3 Matrix::Right() const { return -Left(); }
...
Matrix & operator/=(Matrix & m, float s) { return m *= (1.0f / s); }

Matrix operator+(Matrix m1, const Matrix & m2) { return m1 += m2; }

Matrix operator-(Matrix m1, const Matrix & m2) { return m1 -= m2; }

Matrix operator*(Matrix m1, const Matrix & m2) { return m1 *= m2; }

Matrix operator*(Matrix m, float s) { return m *= s; }
