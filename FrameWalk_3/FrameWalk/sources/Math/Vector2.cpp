...
float Vector2::Length() const { return std::sqrt(Dot(*this, *this)); }

float Vector2::LengthSquared() const { return Dot(*this, *this); }
...
Vector2 Vector2::Normalize(const Vector2 & value) { return Vector2(value).Normalize(); }
...
{
	return Vector2(
		std::min<float>(vector1.x, vector1.x), 
		std::min<float>(vector1.y, vector2.y)
		);
}
...
{
	return Vector2(
		std::max<float>(vector1.x, vector2.x), 
		std::min<float>(vector1.y, vector2.y)
		);
}
...
VECTOR Vector2::Vector2ToVECTOR(const Vector2 & v) { return VGet(v.x, v.y, 0); }
...
Vector2 operator+(Vector2 v1, const Vector2 & v2) { return v1 += v2; }

Vector2 operator-(Vector2 v1, const Vector2 & v2) { return v1 -= v2; }

Vector2 operator*(Vector2 v1, Vector2 v2) { return Vector2(v1.x * v2.x, v1.y * v2.y); }

Vector2 operator*(Vector2 v, float s) { return v *= s; }

Vector2 operator*(float s, Vector2 v) { return v *= s; }

Vector2 operator/(Vector2 v, float s) { return Vector2(); }