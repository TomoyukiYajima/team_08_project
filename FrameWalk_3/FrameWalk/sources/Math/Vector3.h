...
#ifdef __DXLIB
	// DxLibのVECTORをVector3に変換
	// explicit Vector3(DxLib::VECTOR& v) : x(v.x), y(v.y), z(v.z) {}
	// explicit 暗黙の型変換をさせない
	Vector3(DxLib::VECTOR& v) : x(v.x), y(v.y), z(v.z) {}
	// Vector3をDxLibのVECTORに変換
	// 暗黙の型変換
	operator DxLib::VECTOR() const { return{ x, y, z }; }
#endif
