...
{
	return Vector3(
		std::min<float>(vector1.x, vector2.x), 
		std::min<float>(vector1.y, vector2.y), 
		std::min<float>(vector1.z, vector2.z)
		);
}
...
Vector3 Vector3::Max(const Vector3 & vector1, const Vector3 & vector2) 
{
	return Vector3(
		...
		std::max<float>(vector1.y, vector2.y), 
		std::max<float>(vector1.z, vector2.z)
		);
}
...
Vector3 Vector3::Transform(const Vector3 & position, const Matrix & matrix) 
{
	return Vector3(
		...
		position.z * matrix.m[2][0] + matrix.m[3][0],
		position.x * matrix.m[0][1] + position.y * matrix.m[1][1] + 
		position.z * matrix.m[2][1] + matrix.m[3][1],
		position.x * matrix.m[0][2] + position.y * matrix.m[1][2] + 
		position.z * matrix.m[2][2] + matrix.m[3][2]
		);
}
...
Vector3 operator+(Vector3 v1, const Vector3 & v2) { return v1 += v2; }

Vector3 operator-(Vector3 v1, const Vector3 & v2) { return v1 -= v2; }

Vector3 operator*(Vector3 v, float s) { return v *= s; }

Vector3 operator*(float s, Vector3 v) { return v *= s; }
